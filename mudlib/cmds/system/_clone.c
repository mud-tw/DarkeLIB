...
int cmd_clone(string str)
{
    ...
    if (member_group(geteuid(previous_object()), "ambassador"))
        return 0;
    ...
    if(res = catch(ob = new(str = resolv_path((string)this_player()->get_path(), str))))
        write(str+": "+res);
    ...
    if(stringp(ob->query_short()))
        message = replace_string((string)this_player()->query_mclone(),"$O", (string)ob->query_short());
    else message = "";

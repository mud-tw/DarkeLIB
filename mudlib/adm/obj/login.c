...
protected void logon()
{
    ...
    if(catch(__Player = new(OB_USER))) {
        message("logon", "使用者物件正在修改中, 請稍後一會兒再登入.\n", this_object());
...
protected void get_name(string str)
{
    ...
    if (!user_exists(__Name))
    {
        if (!((int)BANISH_D->valid_name(__Name)))
        {
            ...
            message("logon",
                sprintf("\n%s is not a valid name choice for %s.\n",
                    capitalize(__Name), mud_name()
                ),
                this_object()
            );
            message("logon",
                sprintf(
                    "帳號名稱必須為半寫的英文字母, 字數介於 %d 到 %d 之間. \n",
            ...
            message("logon",
                sprintf(
                    "Names must be alphabetic characters no "
            ...
            message("logon", "\n請重新輸入帳號名稱\n", this_object());
            message("logon", "Please enter another name: ", this_object());
            input_to("get_name");
            return;
        }
        ...
        message("logon", sprintf("確定使用 %s 作為您的帳號名稱? (y/n) \n", capitalize(__Name)), this_object());
        message("logon", sprintf("Do you really wish %s to be your name? (y/n) ", capitalize(__Name)), this_object());
        input_to("new_user");
        return;
    }
    if (!((int)BANISH_D->allow_logon(__Name, query_ip_number())))
...
private int is_copy()
{
    ...
    seteuid(UID_LOG);
    log_file("enter", "EXEC:" + __Name + ":" + ctime(time()) + "\n" );
    seteuid(getuid());

    if(exec(ob, this_object()))
        ob->restart_heart();
    else
        message("logon", "Problem reconnecting.\n", this_object());
...
protected void disconnect_copy(string str, object ob)
{
    ...
    message("info", "有人從別的地方連線進來取代你控制角色了!", ob);

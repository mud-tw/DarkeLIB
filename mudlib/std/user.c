...
varargs void move_player(mixed dest, string msg)
{
    ...
    if (this_object()->move(dest) != MOVE_OK)
    {
        ...
        if (this_object()->query("error report"))
        {
            ...
            input_to((
                         : call_other, "/cmds/system/handler", "handle_error"
                         :),
                     0, this_object(),
    ...
    if (!hiddenp(this_object()))
    {
        ...
        for (i = 0, bzbd = sizeof(inv); i < bzbd; i++)
        {
            ...
            if (!living(inv[i]) || inv[i] == this_object())
                continue;
            if (adj)
                tmp = skill_contest(adj, inv[i]->query_skill(
                                             "perception"));
            else
                tmp = 2;
            if (tmp != 2)
                continue;
            if (effective_light(inv[i]) - (adj / 25) < 0)
                continue;
            if (!msg || msg == "")
                message("mmout", query_mmout(), inv[i]);
            else
                message("mout", query_mout(msg), inv[i]);
        }
        ...
        for (i = 0, bzbd = sizeof(inv); i < bzbd; i++)
        {
            ...
            if (effective_light(inv[i]) - (adj / 25) < 0)
                continue;
            if (!msg || msg == "")
                message("mmin", query_mmin(), inv[i]);
            else
                message("min", query_min(), inv[i]);
        }
        if (query_followers())
            move_followers(prev);
    }
...
void remove()
{
    ...
    if (!_quitting && previous_object() && previous_object() != this_object() && geteuid(previous_object()) != UID_ROOT &&
        TP != TO)
        return;
    ...
    if (!hiddenp(this_object()))
        if (_quitting)
            INFORM_D->do_inform("logins_and_quits", "Info: " + capitalize((string)this_object()->query_name()) + " has stepped beyond the boundary of Daybreak Ridge.",
                                who_exc);
...
int finish_quit(object ob)
{
    ...
    log_file("enter", "QUIT:" + (string)this_object()->query_name() + ":" + ctime(time()) + "\n");
...
void setup()
{
    ...
    if (member_array(query_position(), MORTAL_POSITIONS) == -1)
    {
        log_file("security", "Enable Wizard:" +
                                 this_object()->query_name() + ":" + getuid(previous_object()) + ":" + ctime(time()) + ":" + this_object()->query_ip() + ":SETUP\n");
        enable_wizard();
    }
    init_living();
    basic_commands();
    ...
    if (this_player()->query_invis())
        who_exc += filter_array(users(), "filter_notanarch", this_object());
    if (!hiddenp(this_object()))
        INFORM_D->do_inform("logins_and_quits", "Info: " + capitalize((string)this_object()->query_name()) + " steps out of the forest and onto the mountain of Daybreak Ridge.",
                            who_exc);
    ...
    if (query_class() && stringp(query_class()) && query_class() != "child" && file_exists("/d/damned/guilds/join_rooms/" + query_class() + "_join.c"))
    {
            ...
            if (mods)
            {
                ...
                for (i = 0; i < sizeof(mod_keys); i++)
                {
                    if (this_object()->query_base_stats(mod_keys[i]))
                        this_object()->set_stats(mod_keys[i], (int)this_object()->query_base_stats(mod_keys[i]) -
                                                                  mods[mod_keys[i]]);
                }
            }
            set_class("child");
...
varargs protected void heart_beat(int recurs_flag)
{
    ...
    if ((player_age > autosave) && (!wizardp(this_object())))
    {
        ...
        inv = filter_array(inv, (
                                    : ((string)$1->query($2) == $3)
                                    :),
                           "protected by", (string)this_object()->query_name());
        if (sizeof(inv) > 3)
            inv = inv[0..2];
        SAVEALL_D->add_crash_items(this_object());
        if (sizeof(inv))
        {
            message("environment", "%^CYAN%^Items protected: " + implode(map_array(inv, (
                                                                                            : call_other:),
                                                                                   "query_short"),
                                                                         ", ") +
                                       ".",
    ...
    if (sizeof(query_attackers()) && getenv("SCORE") != "off")
        message("my_combat", sprintf("hp: %d (%d)  mp: %d (%d)", query_hp(), query_max_hp(), query_mp(), query_max_mp()), this_object());
    if (stringp(props["lycanthrope moon"]) && !this_object()->query("in creation"))
    {
        tod = (string)EVENTS_D->query_time_of_day();
        if (!this_object()->query_lyc_status() && (((int)ASTRONOMY_D->query_phase(props["lycanthrope moon"]) == 3 && (tod == "night" || tod == "twilight")) ||
                                                   this_object()->query("control change")))
        {
            message("info", "%^RED%^You change into a huge " + capitalize(replace_string((string)this_object()->query_race(), "were", "")) + "!", this_object());
            tell_room(environment(this_object()), capitalize(query_name()) + " turns into a huge " + capitalize(replace_string((string)this_object()->query_race(), "were", "")) + "!", ({this_object()}));
            lyc_ob = new ("/std/spells/shadows/lyc_shadow");
            lyc_ob->start_shadow(this_object());
            this_object()->set("lyc ob", lyc_ob);
            force_me("drop all");
        }
        else if (this_object()->query_lyc_status() && ((int)ASTRONOMY_D->query_phase(props["lycanthrope moon"]) != 3 || (tod != "night" && tod != "twilight")) &&
                 !this_object()->query("control change"))
        {
            message("info", "%^RED%^You cange back to normal form.", this_object());
            tell_room(environment(this_object()), capitalize(query_name()) + " changes back to normal form.", ({this_object()}));
            lyc_ob = (object)this_object()->query("lyc ob");
            if (!objectp(lyc_ob))
                message(
                    "info", "BUG in your lycanthropy.  " + "Cannot dest shadow.", this_object());
            else
                lyc_ob->external_destruct(lyc_ob);
            this_object()->set("lyc ob", 0);
        }
    }
    if (!wizardp(this_object()) && (int)this_object()->query_hp() < 0 && !this_object()->query_ghost())
        this_object()->die();
    if (!disable)
        continue_attack();
    if (magic_round > 0)
...
void set_property(string idx, mixed val)
{
    ...
    if (!idx)
        return;
    if (member_array(idx, LOG_PROPS) >= 0 && !wizardp(this_object()) && origin() != "local" && file_name(previous_object()) != "/d/standard/setter")
    {
        if (this_player())
...
varargs void net_dead(int flag)
{
    ...
    else if (flag && interactive(this_object()))
        return;
    ...
    if (this_object()->query_invis())
        who_exc += filter_array(users(), "filter_notanarch", this_object());
    if (environment() && environment() == load_object(ROOM_FREEZER))
        return;
    if (!hiddenp(this_object()))
        if (!this_object()->query_current_attacker())
            INFORM_D->do_inform("net_dead_players", "Info: " + capitalize((string)this_object()->query_name()) + " goes net dead.", who_exc);
        else
            INFORM_D->do_inform("net_dead_players", "Info: " + capitalize((string)this_object()->query_name()) + " suckered out and went net dead %^BOLD%^%^YELLOW%^IN COMBAT%^RESET%^!!!", who_exc);
    channels = ({});
    ...
    map_array(all_inventory(this_object()), (
                                                : call_other:),
              "set", "protected by", 0);
    SAVEALL_D->add_crash_items(this_object());
...
void restart_heart()
{
    ...
    set_heart_beat(1);
    if (net_died_here)
        this_object()->move_player(net_died_here);
    else
    ...
    if (this_player()->query_invis())
        who_exc += filter_array(users(), "filter_notanarch", this_object());
    if (!hiddenp(this_object()))
        INFORM_D->do_inform("net_dead_players", "Info: " + capitalize((string)this_object()->query_name()) + " recovers from net death.",
                            who_exc);
}
...
nomask void die()
{
    ...
    force_me("unequip");
    message("death", "You die.\nYou feel the sensations of nothingness " + "as you rise above your corpse.\nYou arrive at a destination in a reality " + "not like your own.", this_object());
    while (this_object()->query_paralyzed())
        this_object()->remove_paralyzed();
...
{
    if (geteuid(previous_object()) != UID_ROOT &&
        file_name(previous_object()) != PASSWD_D)
        return 0;
    password = pass;
    save_player(query_name());
}
...
void write_messages()
{
    ...
    if (mail_stat["unread"])
    {
        message("login", sprintf("\n        >>> %d of your %d %s are "
                                 "unread! <<<",
                                 mail_stat["unread"], mail_stat["total"], (mail_stat["total"] > 1 ? "letters" : "letter")),
                this_object());
    }
    if (query_invis())
        message("login", "\n        You are currently "
...
string query_title()
{
    ...
    if (query_ghost())
        return "The ghost of " + capitalize(query_name());
    if (invis)
        return 0;
    str = getenv("TITLE");
    if (!str)
...
void receive_message(string msg_class, string msg)
{
    ...
    if (query_blocked(msg_class) &&
        member_array(msg_class, OVERRIDE_IGNORE_MSG) == -1)
        return;
    switch (msg_class)
    {
    case "room_exits":
    ...
    for (i = 0, max = sizeof(words = explode(msg, "%^")); i < max; i++)
        if (term_info[words[i]])
            words[i] = term_info[words[i]];
...
void set_blocked(string str)
{
    ...
    if (blocked[str])
        message("info", "You are now blocking " + str + ".",
                this_object());
    else
        message("info", "You are no longer blocking " + str + ".", this_object());
}
...
{
    if (!died_here)
        this_object()->move("/d/standard/square");
    else
        this_object()->move(died_here);
}
...
void revive()
{
    ...
    {
        message("my_action", "Your soul has no corpse to gate into " + "the prime material plane!\nYour soul loses energy in " + "escaping.", this_object());
        flag = 1;
    }
    if (flag)
        this_object()->reduce_exp(-1, );
...
void add_quest_points(int pts)
{
    ...
    if (pts > 0)
        log_file("quests", query_name() + " received " + sprintf("%d quest points ", pts) + "from " + file_name(previous_object()) +
                               " (" + getuid(previous_object()) + ")\nDate: " + ctime(time()) + "\n");
    return;
}
...
void divorce_me()
{
    if (!current_marriage)
        return;
    if (!divorced)
        divorced = ({({current_marriage[0], current_marriage[1], time()})});
    else
        divorced += ({({current_marriage[0], current_marriage[1], time()})});
    current_marriage = 0;
}
...
void hide(int x)
{
    if (file_name(previous_object()) != "/cmds/adm/_haven")
        return;
    set_hide(x);
}
...
void set_bank(string str, int val)
{
    if (BN(PO)[0..28] != "/d/damned/akkad/obj/mon/silas")
        return;
    if (!bank)
        bank = ([]);
    if (val < 1)
...
void convert_kills()
{
    ...
    if (!player_data["kills"] || !sizeof(player_data["kills"]) ||
        intp(player_data["kills"][0]))
        return;
    tmp = ({});
    i = sizeof(player_data["kills"]);
    while (i--)
        tmp += ({(int)PLAYER_D->add_kill(player_data["kills"][i])});
...
int register_channels()
{
    ...
    if (wizardp(this_object()))
    {
        ...
        channels = ({});
        while (i--)
            if (sscanf(dir[i], "%s_join.c", class_name))
                channels += ({class_name, class_name + "-council"});
    ...
    if (gm)
        gm = gm->query_master();
    if (this_player()->query_name() == gm)
        channels += ({"council", this_object()->query_class() + "-council"});

    if (wizardp(this_object()) ||
        high_mortalp(this_object()))
        channels += ({"newbie", "high-mortal"});
    if (legendp(this_object()))
        channels += ({"legend"});
    else
        channels += ({"newbie"});
    if (wizardp(this_object()) ||
        (int)this_object()->query_level() > 3)
        channels += ({"daybreak"});
    if (wizardp(this_object()))
        channels += ({"cre", "intercre", "darkelib", "legend"});
    if (archp(this_object()))
        channels += ({"admin", "council"});
    if (!wizardp(this_object()) && this_object()->query_class())
    {
        join_room = "/d/damned/guilds/join_rooms/" +
                    (string)this_object()->query_class() + "_join";
        if (file_exists(join_room + ".c") && (string)join_room->query_master() == (string)this_object()->query_name() && wizardp(this_object()))
            channels += ({"council", this_object()->query_class() + "-council"});
        if (file_exists(join_room + ".c") && (int)join_room->filter_council(this_object()->query_true_name()) == 1)
            channels += ({this_object()->query_class() + "-council"});
        if ((string)GUILD_D->query_gm(query_class()) ==
            (string)this_object()->query_name())
            channels += ({"new-council"});
    }
...
void clean_net_dead()
{
    if (base_name(previous_object()) != ROOM_FREEZER)
        return;
    this_object()->remove();
}

...
void init()
{
    ...
    if ((this_player()->query_invis() && !this_object()->query("see invis")) ||
        skill_contest((int)this_object()->query_skill("perception"), (int)this_player()->query_stealth(), 1) != 1)
        return;
...
void chase(object plyr)
{
    if (plyr == this_object()->query_following())
        return;
...
varargs void heart_beat(int recurs_flag)
{
    ...
    if (emotes[pre + "chance"] > random(100))
        do_emote(pre);
    if (mapp(spell_chance) && sizeof(keys(spell_chance)))
        do_spell();
    continue_attack();
...
{
    if (!this_object()->query_spell_level(spell))
        return 0;
    if (!spell_chance || !spell_chance[spell])
        return 0;
    ...
    if (spell_chance[spell + " delay"])
    {
        ...
        if (spell_delays[spell] && (time() - spell_delays[spell]) <
                                       spell_chance[spell + " delay"])
            return 0;
    }
    if (((int)this_object()->query_max_hp() -
         (int)this_object()->query_hp()) > 20)
        return 1;
...
void do_spell()
{
    ...
    z = sizeof(spell_keys);
    if (query_current_attacker())
        cast_chance = combat_chance;
    else
        cast_chance = casting_chance;
    ...
    if (z && random(300) < cast_chance && !this_object()->query_casting())
    {
        ...
        seteuid(getuid());
        if (file_exists(spell_file + ".c"))
            spell_ob = new (spell_file);
        if (spell_ob)
        {
            random_dudes = filter_array(all_inventory(environment(
                                            this_object())),
                                        "not_party", spell_ob, this_object());
...
{
    if (!stringp(spell) || !stringp(cast_string))
        return;
    if (!mapp(spell_chance))
        spell_chance = ([]);
    spell_chance[spell] = cast_string;
    if (delay)
        spell_chance[spell + " delay"] = delay;
    if (combat)
    {
        if (!combat_spells)
            combat_spells = ({});
    ...
    if (query("gang bang") && which != 2 && (who->is_player() || who->is_pet()))
    {
        while (i--)
        {
            if (inv[i]->is_player() || inv[i]->is_pet())
                continue;
            if (query_attackers() && member_array(inv[i], query_attackers()) >= 0)
                continue;
            inv[i]->kill_ob(who, 2);
        }
    }
...
void set_level(int x)
{
    ...
    level = x;
    if (x >= 10 && x <= 20)
        set_skill("resist stun", (x - 9) * 5 + 10);
    else if (x > 20)
        set_skill("resist stun", 60 + 3 * (x - 20));
    ...
    if (x > 10)
        set_property("magic resistance", (x - 10) * 5);
...
void set_melee_damage(mixed what)
{
    ...
    if (!mapp(what))
    {
        if (member_array(what, DAMAGE_TYPES) < 0)
            return;
    ...
    while (i--)
    {
        if (member_array(tmp[i], DAMAGE_TYPES) < 0)
            map_delete(what, tmp[i]);
    }
    if (!mapp(props["melee damage"]))
        props["melee damage"] = ([]);
...
void receive_message(string cl, string msg)
{
    ...
    else if (sscanf(msg, "%s yells in %*s: %s,%s", who, me, line) == 4 && id(lower_case(me)))
    {
        from = find_player(lower_case(who));
        if (!from)
            from = present(lower_case(who), environment());
        if (!from)
            from = find_living(lower_case(who));

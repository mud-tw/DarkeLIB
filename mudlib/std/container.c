...
string describe_living_contents(object *exclude)
{
    ...
    i = sizeof(
        inv = filter_array(
            all_inventory( this_object() ) - exclude,
            (: living :)
        )
    );
    ...
    while(i--) {
        ...
        if (inv[i]->query_hiding()
            && skill_contest((int)inv[i]->query_hiding(), x, 1) != 2
        ...
        if (inv[i]->query_invis()
            && !previous_object()->query("see invis")
        ) continue;
        // if(!tmp); else
        if (tmp) {
...
string describe_item_contents(object *exclude)
{
    ...
    if(!i)
        ...
        return ret + " and " + consolidate(sizeof(list[shorts[0]]), shorts[0]) + " are here.\n";
    else {
        while(i--) {
            if(!i) ret += ", and ";
            else ret += ", ";

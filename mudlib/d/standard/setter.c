...
string rand_choice = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";
...
int pick(string str)
{
    ...
    while(i--)
        call_other("/d/damned/guilds/join_rooms/"+g_dir[i], "kick_member", name);
    ...
    who->add_sight_bonus(
        (int)RACE_D->query_sight_bonus(str) - (int)who->query_sight_bonus()
    );
...
void assign_point(string str, int *points, int pts_left)
{
    ...
    printf(CLS);
    write(sprintf("%|70s\n%|70s\n", "Stat Assignment", sprintf("points left: %d", pts_left)));
    max = sizeof(tab_str);
    write(sprintf("%'-='70s", ""));
    for (i = 0; i < max; i++) {
        if (i < sizeof(STATS))
            write(
                sprintf("  %-18s     %-13s %3d %-+2d (%d points)",
                    tab_str[i], STATS[i] + ":", pt_to_stat(points[i]),
                    (int)RACE_D->query_stat_adjustment(
                        STATS[i][0..2],
...
void yes_or_no(string str, int *points, int pts_left)
{
    ...
    if (!this_player()->query_property("ambidextry"))
        write("IMPORTANT:  You favor your " + (string)this_player()->query_property("handedness") + ".  Wield weapons there\n" +
              "when possible.");
    who->set_lang_prof("common", 10 + (int)this_player()->query_stats("intelligence") / 20);
    ...
    dev_tot += 9;
    if (dev_tot < 55)
        message("info", "Your stats suck.  If you would like to be deleted, " + "ask an Arch immediately!", who);
